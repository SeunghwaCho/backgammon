...
export function createInitialBoard(): Point[] {
  const board: Point[] = [];
...
export function cloneBoard(board: Point[]): Point[] {
...
export function bearOffTarget(player: Player): number {
...
export function getBarCount(state: GameState, player: Player): number {
  ...
  if (pt.owner === player) return pt.count;
  return 0;
}

// Count total checkers for a player (board + bar + borne off)
export function countTotalCheckers(state: GameState, player: Player): number {
  let total = 0;
  for (let i = 0; i <= 25; i++) {
    const pt = state.board[i];
    if (pt.owner === player) total += pt.count;
  }
  if (player === 'white') total += state.whiteBorneOff;
  else total += state.blackBorneOff;
  return total;
}

// Verify checker counts are correct (15 per player)
export function verifyCheckerCounts(state: GameState): boolean {
  return (
    countTotalCheckers(state, 'white') === 15 &&
