...
import { GameState, Player, Move, Point } from '../game/Types.js';
...
export class CanvasRenderer {
  ...
  private layout: RendererLayout | null = null;
  ...
  getPointCenter(pointIndex: number): { x: number; y: number } | null {
    if (!this.layout) return null;
    const l = this.layout;
  ...
  private getLandscapePointCenter(pointIndex: number): { x: number; y: number } | null {
    if (!this.layout) return null;
    const l = this.layout;
  ...
  private getPortraitPointCenter(pointIndex: number): { x: number; y: number } | null {
    if (!this.layout) return null;
    const l = this.layout;
  ...
  private getPointX(pointIndex: number): { x: number; isTop: boolean } {
  ...
  private hitTestLandscape(sx: number, sy: number): number | null {
    if (!this.layout) return null;
    const l = this.layout;

    // Bear-off area clicks (outside main board)
    if (sx >= l.whiteBearOffX && sx < l.whiteBearOffX + l.bearOffW) {
      if (sy >= l.boardY && sy <= l.boardY + l.boardH) return -1; // white bear off
    }
  ...
  private hitTestPortrait(sx: number, sy: number): number | null {
    if (!this.layout) return null;
    const l = this.layout;
  ...
  private renderPortrait(state: GameState): void {
    const ctx = this.ctx;
    const l = this.layout!;
  ...
  private drawLandscapePoint(pointIndex: number, state: GameState): void {
    const ctx = this.ctx;
    const l = this.layout!;
    const { x, isTop } = this.getPointX(pointIndex);
  ...
  private drawPortraitPoint(pointIndex: number, state: GameState): void {
    const ctx = this.ctx;
    const l = this.layout!;
    ...
    let col: number, isTop: boolean, baseY: number, tipY: number, centerX: number;
    const midY = l.boardY + l.boardH / 2;
  ...
  private drawCheckersOnPoint(pointIndex: number, state: GameState): void {
    if (!this.layout) return;
    const l = this.layout;
  ...
  private drawBarCheckers(state: GameState): void {
    if (!this.layout) return;
    const l = this.layout;
    ...
    for (const player of ['white', 'black'] as Player[]) {
      ...
      if (isSelectable || isSelected) {
        ...
        if (l.isPortrait) {
          ...
          ctx.globalAlpha = 0.4;
          ctx.fillRect(
            l.barX, l.boardY + l.boardH / 2 - l.checkerR * 2.5,
            l.barW, l.checkerR * 5
          );
        }
        ctx.restore();
  ...
  private drawBarValidTargets(state: GameState): void {
    if (!this.layout) return;
    const l = this.layout;
  ...
  private drawBearOffAreasPortrait(state: GameState): void {
    if (!this.layout) return;
    const l = this.layout;
    const ctx = this.ctx;
    const bh = l.bearOffW;
  ...
  renderValidTargets(state: GameState): void {
    if (!this.layout) return;
    const l = this.layout;
    const ctx = this.ctx;
  ...
  private drawBearOffHighlight(to: number, player: Player): void {
    if (!this.layout) return;
    const l = this.layout;
    const ctx = this.ctx;
