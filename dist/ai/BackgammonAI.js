...
import { getPipCount, getHomeBoardRange } from '../game/Rules.js';
...
function evaluatePosition(board, player, whiteBorneOff, blackBorneOff) {
    ...
    for (let i = 1; i <= 24; i++) {
        ...
        if (pt.owner === player && pt.count === 1)
            myBlots++;
        if (pt.owner === opponent && pt.count === 1)
            oppBlots++;
    }
    score -= myBlots * 2;
    score += oppBlots * 1.5;
    ...
    for (let i = 1; i <= 24; i++) {
        ...
        if (pt.owner === player && pt.count >= 2)
            myPoints++;
        if (pt.owner === opponent && pt.count >= 2)
            oppPoints++;
    }
    score += myPoints * 1.5;
    // Home board progress
    ...
    if (myBarPt.owner === player)
        score -= myBarPt.count * 8;
    if (oppBarPt.owner === opponent)
        score += oppBarPt.count * 5; // opponent on bar is good for us
    ...
    if (player === 'black') {
        for (let i = 1; i <= 6; i++) {
            if (board[i].owner === 'black' && board[i].count >= 2)
                score += 2;
        }
    }
    else {
        for (let i = 19; i <= 24; i++) {
            if (board[i].owner === 'white' && board[i].count >= 2)
                score += 2;
        }
    }
    return score;
...
export function chooseBestSequence(state) {
    ...
    for (const seq of validSeqs) {
        ...
        const score = evaluatePosition(resultState.board, player, resultState.whiteBorneOff, resultState.blackBorneOff);
...
export async function executeAITurn(state, onMove, onComplete, rollDiceFn) {
    // First roll dice
    await delay(400);
...
//# sourceMappingURL=BackgammonAI.js.map