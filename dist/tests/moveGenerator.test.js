...
import { generateAllLegalSequences, getSelectablePoints, getMovesFromPoint, filterSequencesAfterMove, } from '../game/MoveGenerator.js';
/** Helpers */
function emptyBoard() {
    return Array.from({ length: 26 }, () => ({ owner: null, count: 0 }));
}
function place(board, index, owner, count) {
    board[index] = { owner, count };
}
function makeDice(a, b) {
    if (a === b) {
...
describe('filterSequencesAfterMove', () => {
    test('filters sequences to those starting with the given move', () => {
        ...
        for (const seq of filtered) {
            ...
            if (seq.length > 0) {
                assert.ok(!(seq[0].from === firstMove.from && seq[0].to === firstMove.to), 'Filtered sequence should not repeat the first move');
            }
        }
    });
    test('returns empty when move not in any sequence', () => {
        ...
        place(board, 10, 'white', 2);
        const seqs = generateAllLegalSequences(board, 'white', makeDice(2, 4), 0);
        const fakeMove = { from: 10, to: 3, dieUsed: 7, isHit: false };
...
//# sourceMappingURL=moveGenerator.test.js.map