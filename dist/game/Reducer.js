...
import { cloneGameState, createInitialGameState, createInitialCube } from './GameState.js';
...
export function canOfferDouble(state) {
    if (state.phase !== 'waitingForRoll')
        return false;
    if (state.match.isCrawford)
        return false;
    if (state.cube.value >= 64)
        return false;
    if (state.cube.owner !== null && state.cube.owner !== state.currentPlayer)
        return false;
    return true;
}
...
export function offerDouble(state) {
...
export function acceptDouble(state) {
    if (state.phase !== 'playerDecidingDouble')
        return state;
    const next = cloneGameState(state);
...
export function declineDouble(state) {
    if (state.phase !== 'playerDecidingDouble')
        return state;
    const next = cloneGameState(state);
...
export function startNextGame(state) {
    ...
    if (eitherAtMatchPoint && !oldMatch.postCrawford && !oldMatch.isCrawford) {
        ...
        if (oldMatch.isCrawford)
            oldMatch.postCrawford = true;
