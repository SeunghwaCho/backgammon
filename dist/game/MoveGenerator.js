...
import { isSingleMoveValid, applySingleMove, } from './Rules.js';
...
function applySimMove(simState, player, from, to, dieValue) {
...
export function generateAllLegalSequences(board, player, dice, borneOff) {
    const initialState = {
    ...
    if (!isDoubles && maxDiceUsed === 1 && dice.remaining.length === 2) {
        ...
        const maxDieInSequences = Math.max(...maxDiceSequences.map(seq => (seq.length > 0 ? seq[0].dieUsed : 0)));
        // Only keep sequences using the highest die
        const higherDieSeqs = maxDiceSequences.filter(seq => seq.length > 0 && seq[0].dieUsed === maxDieInSequences);
        if (higherDieSeqs.length > 0)
            return deduplicate(higherDieSeqs);
    }
...
export function getSelectablePoints(sequences) {
    const points = new Set();
    for (const seq of sequences) {
        if (seq.length > 0)
            points.add(seq[0].from);
    }
    return points;
}
// Get all valid first moves from a specific source point
export function getMovesFromPoint(sequences, from) {
...
export function filterSequencesAfterMove(sequences, move) {
    const filtered = [];
    for (const seq of sequences) {
        if (seq.length > 0) {
            const first = seq[0];
