...
import { rollDice as reducerRollDice, rollForFirst as reducerRollForFirst, selectPoint as reducerSelectPoint, applyPlayerMove, applyMoveInternal, startNewGame, startNextGame, restoreFromSave, offerDouble, acceptDouble, declineDouble, canOfferDouble, } from './game/Reducer.js';
...
import { renderButtons, renderRestorePrompt, renderNewGameConfirm, renderClearSaveConfirm, renderDoubleOfferPrompt } from './ui/HUD.js';
...
function playGameOverSound(state, _perspective = 'player') {
...
function aiShouldDouble(state) {
    ...
    if (totalMoved < 2)
        return false; // too early to double
    if (bPip === 0)
        return false; // almost done, no need to double
...
function renderStartupPrompt() {
    ...
    restoreButtons = renderRestorePrompt(ctx, canvas.clientWidth, canvas.clientHeight, savedData.timestamp, fontScale);
