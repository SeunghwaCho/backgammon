...
export class CanvasRenderer {
        ...
        if (state.phase !== 'gameOver') {
            ...
            if (state.phase === 'rollingForFirst') {
                if (!state.initialRoll)
                    infoMsg = loc.rollForFirstPrompt;
                else if (state.initialRoll.white === state.initialRoll.black)
                    infoMsg = loc.rollForFirstTie;
                else
                    infoMsg = state.currentPlayer === 'white' ? loc.rollForFirstWhiteFirst : loc.rollForFirstBlackFirst;
            }
    ...
    drawDice(state) {
        ...
        if (this.diceRollAnim &&
            performance.now() - this.diceRollAnim.startTime < this.diceRollAnim.duration)
            return;
        const l = this.layout;
        const ctx = this.ctx;
    ...
    drawDie(x, y, size, value, isWhite, used, alpha = 1.0) {
        const ctx = this.ctx;
        const r = size * 0.15;
